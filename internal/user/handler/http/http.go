...
var (
	...
	// HandlerForgotPassword denotes HTTP handler for user to
	// interact with user password data.
	HandlerForgotPassword = HandlerIdentity{
...
func (h *Handler) createHTTPHandler(configName string) (http.Handler, error) {
	...
	case HandlerForgotPassword.Name:
		httpHandler = &forgotPasswordHandler{
			user: h.user,
		}
